...
void HTTPCache::aquireLock(const HTTPRequest& request) {
    string hash = hashRequest(request);
    map_lock.lock();
    unique_ptr<mutex>& lock = cache_locks[hash];
    map_lock.unlock();
    if (lock == NULL) {
...
void HTTPCache::releaseLock(const HTTPRequest& request) {
    string hash = hashRequest(request);
    map_lock.lock();
    unique_ptr<mutex>& lock = cache_locks[hash];
    lock->unlock();
...
bool HTTPCache::containsCacheEntry_ts(const HTTPRequest& request, HTTPResponse& response) {
    aquireLock(request);
    bool r = containsCacheEntry(request, response);
    releaseLock(request);
    return r;
}
...
void HTTPCache::cacheEntry_ts(const HTTPRequest& request, const HTTPResponse& response) {
    aquireLock(request);
